...
Scale = class 'Scale' {
  ...
  to_scale_index = function(self, pitch)
    ...
    local normalized_indices = transform(self:get_pitches(), function(pitch)
  ...
  contains = function(self, other)
    ...
    function canonicalize(pitch_indices, octave_interval)
      return transform(pitch_indices, function(pitch_index)
  ...
  while true do
    for root_scale_index in range(start, finish, direction) do
      local absolute_scale_indices = 
        transform(relative_scale_indices, function(scale_index)
